...
from tokenserver.test.server_oauth2 import app
...
async def test_oauth2_authorize(startup_and_shutdown_oauth2_server):
    ...
    uri, state = client.create_authorization_url('http://127.0.0.1:12345/oauth/authorize')
...
async def test_signin(startup_and_shutdown_oauth2_server):
    async with httpx.AsyncClient() as client:
        ...
        authorization_url, state = oauth_client.create_authorization_url('http://localhost:12345/oauth/authorize')
        ...
        response_token = await oauth_client.fetch_token("http://127.0.0.1:12345/oauth/token", authorization_response=resp_post.headers['location'])
        assert response_token is not None

        # Fetch authenticated resource
        is_me = await oauth_client.get('http://127.0.0.1:12345/api/me.json')
